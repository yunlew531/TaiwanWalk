<template>
  <div class="main-container mx-auto">
    <nav class="text-green-100 mx-12 mt-16 mb-8">
      ...
      <span class="text-green-300 cursor-auto">{{ activity.ActivityName }}</span>
    </nav>
    <main>
      ...
      <section class="flex items-start mx-12 mb-16">
        ...
        <div class="w-1/2">
          ...
          <Surround :position="activity.Position" />
        </div>
      </section>
      <MoreList
        title="還有這些不能錯過活動"
...
export default {
  ...
  async asyncData ({ $axios, params, $translateCity }) {
    ...
    let moreActivities = []
    try {
      const res = await $axios.get(`/v2/Tourism/Activity?$filter=contains(ActivityID, '${id}')`)
      activity = res.data[0]
